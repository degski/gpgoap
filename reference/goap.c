...
void goap_actionplanner_init( actionplanner_t* ap )
{
	ap->numatoms = 0;
	ap->numactions = 0;
	...
	for ( int i=0; i<MAXACTIONS; ++i )
	{
		...
		goap_worldstate_init( ap->act_pre+i );
		goap_worldstate_init( ap->act_pst+i );
	}
}


void goap_worldstate_init( worldstate_t* ws )
{
	ws->values = 0LL;
	ws->dontcare = -1LL;
}
